...
/*
  Description: this script generates a file with the extrated data of Fortishield API 4.0, formatted to use in Fortishield app.

  Requirements:
    - Fortishield manager 4.0 up

  Use: node generate-api-info.js FORTISHIELD_API_URL [-f filename]
...
// Constants
const FORTISHIELD_API_URL = process.argv[2] // Fortishield API url is a required argument
...
// Main method
const main = () => {
  // Check Fortishield API url argument is defined
  if (!FORTISHIELD_API_URL) {
    exitWithMessage('Fortishield API url is not defined.')
  }
  // Check Fortishield API url argument is valid
  if (!FORTISHIELD_API_URL.startsWith('http')) {
    exitWithMessage(
      `Fortishield API url is not valid. It should start with "http". Example: https://172.16.1.2:55000`
    )
  }

  // Log the configuration:
  console.log('--------------- Configuration ---------------')
  console.log(`Fortishield API url: ${FORTISHIELD_API_URL}`)
...
const generateAPIEndpointsInformation = async () => {
  try {
    ...
    const apiData = await request(`${FORTISHIELD_API_URL}/openapi.json`)
...
const generateAPISecurityActionsInformation = async () => {
  ...
  const username = 'fortishield'
  const password = 'fortishield'
  try {
    const authenticationResponse = await request(
      `${FORTISHIELD_API_URL}/security/user/authenticate`,
      {
        method: 'POST',
        headers: {
          Authorization:
    ...
    const securityActionsResponse = await request(
      `${FORTISHIELD_API_URL}/security/actions`,
      {
        headers: {
...
// Formatters
// Format the endpoint to use in the Fortishield app
...
const generateEndpointDocumentationLink = (endpointData) =>
  `${FORTISHIELD_DOCUMENTATION_API_REFERENCE_URL}#operation/${endpointData.operationId}`
