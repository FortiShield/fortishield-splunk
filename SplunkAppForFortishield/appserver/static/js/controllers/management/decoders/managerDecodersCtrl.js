define(['../../module', '../rules/ruleset'], function (controllers, Ruleset) {
  ...
  class Decoders extends Ruleset {
    ...
    $onInit() {
      ...
      this.scope.$broadcast('fortishieldSearch', { term: '', removeFilters: true })
        ...
        try {
          if (window.localStorage.decoders) {
            ...
            if (this.filter.length > 0)
              this.scope.$broadcast('fortishieldFilter', { filter: this.filter })
          }
        } catch (err) {
    ...
    onlyParents(typeFilter) {
      ...
      if (typeFilter === 'all') {
        ...
        this.scope.$broadcast('fortishieldUpdateInstancePath', { path: '/decoders' })
      } else {
