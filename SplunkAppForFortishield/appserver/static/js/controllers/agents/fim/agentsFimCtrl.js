  ...
  class AgentsFim extends DashboardMain {
    ...
    $onInit() {
      ...
      this.scope.search = (term) => {
        this.scope.$broadcast('fortishieldSearch', { term })
      }
      ...
      this.scope.loadVizz = () => {
        if (!this.vizzLoaded) {
          this.vizz = [
            ...
            new AreaChart(
              'eventsOverTimeElement',
              `${this.filters} sourcetype="fortishield"  "rule.groups{}"="syscheck" | timechart span=12h count by rule.description`,
              'eventsOverTimeElement',
              this.scope,
            ...
            new ColumnChart(
              'topGroupOwnersElement',
              `${this.filters} sourcetype="fortishield" uname_after syscheck.gname_after!=""| top limit=20 "syscheck.gname_after"`,
              'topGroupOwnersElement',
              this.scope
            ),
            new PieChart(
              'topUserOwnersElement',
              `${this.filters} sourcetype="fortishield" uname_after| top limit=20 "syscheck.uname_after"`,
              'topUserOwnersElement',
              this.scope
            ),
            new PieChart(
              'topActions',
              `${this.filters} sourcetype="fortishield" | stats count by "syscheck.event"`,
              'topActions',
              this.scope
            ),
            new PieChart(
              'topFileChangesElement',
              `${this.filters} sourcetype="fortishield" "Integrity checksum changed" location!="syscheck-registry" syscheck.path="*" | top syscheck.path`,
              'topFileChangesElement',
              this.scope
            ),
            new PieChart(
              'rootUserFileChangesElement',
              `${this.filters} sourcetype="fortishield" "Integrity checksum changed" location!="syscheck-registry" syscheck.path="*" | search root | top limit=10 syscheck.path`,
              'rootUserFileChangesElement',
              this.scope
            ),
            new PieChart(
              'wordWritableFilesElement',
              `${this.filters} sourcetype="fortishield" rule.groups{}="syscheck" "syscheck.perm_after"=* | top "syscheck.perm_after" showcount=false showperc=false | head 1`,
              'wordWritableFilesElement',
              this.scope
            ),
            new Table(
              'eventsSummaryElement',
              `${this.filters} sourcetype="fortishield" rule.groups{}="syscheck"  |stats count sparkline by agent.name, syscheck.path syscheck.event, rule.description | sort count DESC | rename agent.name as Agent, syscheck.path as File, syscheck.event as Event, rule.description as Description, count as Count`,
              'eventsSummaryElement',
              this.scope
            ),
