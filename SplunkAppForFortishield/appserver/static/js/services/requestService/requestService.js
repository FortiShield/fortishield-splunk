  ...
  module.service('$requestService', [
    ...
    function ($https, $apiIndexStorageService, $q, $apiResponseModelFactory) {
      ...
      const getWellFormedUri = (endpoint) => {
        try {
          return `${getBaseUrl()}en-US/custom/SplunkAppForFortishield/${endpoint}`
        } catch (err) {
      ...
      /**
       * Performs a GET request to Fortishield API
       * @param {String} endpoint
       * @param {Object} opts
       *
       * @see apiReqInModel
      ...
      const apiReq = async (endpoint, opts = null, method = 'GET') => {
        try {
          ...
          if (
            ...
            throw new Error('ERROR3099 - Fortishield not ready yet.')
          }
          return result
        } catch (err) {
      ...
      const fortishieldIsReady = async (opts = null) => {
        try {
          $https.defaults.headers.post['Content-Type'] =
            'application/x-www-form-urlencoded'
          const currentApi = $apiIndexStorageService.getApi()
          const apiId =
            currentApi && currentApi['_key'] ? currentApi['_key'] : opts['_key']
          const endpoint = '/api/fortishield_ready'
          const method = 'GET'
      ...
      /**
       * Performs a request to the Fortishield API and returns its response in a model.
       * @param {String} endpoint
      ...
      const service = {
        ...
        fortishieldIsReady: fortishieldIsReady,
