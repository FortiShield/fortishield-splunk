  ...
  module.run([
    ...
    function (
      ...
      async function checkVersions() {
        ...
        // Check that the Fortishield API version matches the App's version.
        const appVersion = $appVersionService.getAppInfo().version
        await checkFortishieldVersions(appVersion)
        ...
        // Utility inner function.
        async function checkFortishieldVersions(appVersion) {
          try {
            ...
            if (result.data && result.data.data && !result.data.error) {
              ...
              // Show toast on versions mismatch.
              // @sends_event FORTISHIELD_VERSION_MISMATCH
              if (appVersion !== APIversion) {
                $rootScope.$broadcast('FORTISHIELD_VERSION_MISMATCH', {
                  appVersion,
                  APIversion,
                })
              }
      ...
      async function checkBeforeTransition(state) {
        try {
          ...
          $rootScope.fortishieldNotReadyYet = false
          $rootScope.fortishieldCouldNotBeRecovered = false
        } catch (err) {
          if (
            ...
            $rootScope.$broadcast('fortishieldNotReadyYet', {})
      ...
      // Check secondary states when Fortishield is not ready to prevent
      // changing the state.
      $transitions.onBefore({}, async (trans) => {
        ...
        if (
          ...
          const fortishieldIsReady =
            window.localStorage.getItem('fortishieldIsReady') === 'true'
          if (!fortishieldIsReady) {
            return false
          }
        }
      })
      ...
      // When access to a state and Fortishield is not ready is detected, this
