/*
 * Fortishield app - Fortishield table directive
...
define([
  ...
  'use strict'
  app.directive('fortishieldTableServerSide', function (BASE_URL) {
    return {
      restrict: 'E',
      scope: {
        path: '=path',
      ...
      controller(
        ...
        $scope.totalItems = 0
        $scope.fortishieldTableLoading = true
        $scope.items = []
        ...
        const fetch = async (
          ...
          try {
            ...
            $scope.$emit('fortishieldFetched', { items: $scope.items })
            return
          } catch (error) {
            if (
              error &&
        ...
        const query = async (query, search) => {
          ...
          $scope.fortishieldTableLoading = false
          $scope.$applyAsync()
        }
        ...
        $scope.$on('fortishieldUpdateInstancePath', (event, parameters) =>
          listeners.fortishieldUpdateInstancePath(parameters, instance, init)
        )

        $scope.$on('fortishieldFilter', (event, parameters) =>
          listeners.fortishieldFilter(parameters, filter)
        )

        $scope.$on('fortishieldSearch', (event, parameters) =>
          listeners.fortishieldSearch(parameters, instance, search)
        )

        $scope.$on('fortishieldSort', (event, parameters) =>
          $scope.sort(parameters.field)
        )

        $scope.$on('fortishieldRemoveFilter', (event, parameters) =>
          listeners.fortishieldRemoveFilter(
            parameters,
            instance,
            $tableFilterService,
            init
        ...
        $scope.$on('fortishieldQuery', (event, parameters) => {
          $scope.fortishieldTableLoading = true
          listeners.fortishieldQuery(parameters, query)
        })

        $scope.$on('fortishieldPlayRealTime', () => {
          realTime = true
          return realTimeFunction()
        })

        $scope.$on('fortishieldStopRealTime', () => {
          realTime = false
          return init()
        })
      ...
      templateUrl:
        BASE_URL +
        '/static/app/SplunkAppForFortishield/js/directives/wz-table-server-side/wz-table-server-side.html',
    }
  })
})
