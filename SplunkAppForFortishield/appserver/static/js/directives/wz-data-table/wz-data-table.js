/*
 * Fortishield app - Fortishield table with data as input parameter directive
  ...
  app.directive('wzDataTable', function (BASE_URL) {
    return {
      ...
      controller($scope, $filter, $notificationService, $window) {
        ...
        $scope.totalItems = 0
        $scope.fortishield_table_loading = true
        $scope.items = []
        /**
        ...
        const init = async () => {
          ...
          $scope.fortishield_table_loading = true
          await fetch()
          $scope.fortishield_table_loading = false
        }
        /**
      ...
      templateUrl:
        BASE_URL +
        '/static/app/SplunkAppForFortishield/js/directives/wz-data-table/wz-data-table.html',
    }
  })
})
