...
define([
  ...
  'use strict'
  app.directive('fortishieldTable', function (BASE_URL) {
    return {
      restrict: 'E',
      scope: {
        path: '=path',
      ...
      controller(
        ...
        const fetch = async (options = {}) => {
          try {
            ...
            $scope.searchTable()
            $scope.$emit('fortishieldFetched', { items })
            return
          } catch (error) {
            if (
              error &&
      ...
      templateUrl:
        BASE_URL +
        '/static/app/SplunkAppForFortishield/js/directives/wz-table/wz-table.html',
    }
  })
})
