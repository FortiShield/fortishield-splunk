/*
 * Fortishield app - Fortishield table directive event listeners
...
define([], function () {
  'use strict'
  return {
    fortishieldUpdateInstancePath: (parameters, instance, init) => {
      instance.filters = []
      instance.path = parameters.path
      return init()
    },

    fortishieldFilter: (parameters, filter) => {
      return filter(parameters.filter)
    },

    fortishieldQuery: (parameters, query) => {
      return query(parameters.query, parameters.search)
    },

    fortishieldSearch: (parameters, instance, search) => {
      if (
        parameters &&
    ...
    fortishieldRemoveFilter: (parameters, instance, $tableFilterService, init) => {
