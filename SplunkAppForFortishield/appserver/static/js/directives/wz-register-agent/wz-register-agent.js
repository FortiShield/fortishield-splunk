/*
 * Fortishield app - Fortishield no configuration card directive
    ...
    return {
      ...
      controller($scope, $notificationService, $requestService) {
        ...
        $scope.getVersion = async () => {
          $scope.fortishieldVersion = await $requestService.apiReq('/manager/info')
          $scope.fortishieldVersion =
            (
              ((($scope.fortishieldVersion || {}).data || {}).data || {})
                .affected_items[0] || {}
            ).version || {}
          $scope.fortishieldVersion = $scope.fortishieldVersion.replace('v', '')
          $scope.$applyAsync()
        }
      ...
      templateUrl:
        BASE_URL +
        '/static/app/SplunkAppForFortishield/js/directives/wz-register-agent/wz-register-agent.html',
    }
  })
})
