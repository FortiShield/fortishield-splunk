# -*- coding: utf-8 -*-
"""
Fortishield app - Manager backend.
...
from fortishield_api import Fortishield_API
...
class manager(controllers.BaseController):
    ...
    def polling_state(self, **kwargs):
        ...
        try:
            ...
            app = cli.getConfStanza(
                'inputs',
                'script:///opt/splunk/etc/apps/SplunkAppForFortishield/bin/get_agents_status.py'
            )
    ...
    def get_api(self, **kwargs):
        """
        Obtain Fortishield API from DB.
    ...
    def get_apis(self, **kwargs):
        """
        Obtain all Fortishield APIs from DB.
    ...
    def remove_api(self, **kwargs):
        """
        Delete Fortishield API from DB.
            ...
            parsed_data = jsonbak.dumps(
                {
                    ...
                    'logs_path': make_splunkhome_path(
                        ['var', 'log', 'splunk', 'SplunkAppForFortishield.log'])
                }
            )
        except Exception as e:
    ...
    def check_connection(self, **kwargs):
        ...
        try:
            self.check_fortishield_version(api)
            # Raise error if Fortishield Daemons are not ready.
            ...
            if e == "DAEMONS-NOT-READY":
                self.logger.error(
                    "manager: Cannot connect to API; Fortishield not ready yet.")
                return jsonbak.dumps(
                    {
                        "status": 200,
                        "error": 3099,
    ...
    def upload_file(self, **kwargs):
        ...
        try:
            ...
            # fortishield_api.py verifies the "origin" of the request to be "raw" when a binary file is sent
    ...
    def check_fortishield_version(self, api: API_model):
        """
        Check Fortishield version.

        Compares the App's Fortishield version with the API's Fortishield version.
        ...
        """
        self.logger.debug("manager::check_fortishield_version() called")
        ...
        try:
            ...
            if api_version != app_version:
                raise Exception(
                    "Unexpected Fortishield version. App version: %s, API version: %s"
                    % (app_version, api_version))
        except Exception as e:
            self.logger.error("Error checking the Fortishield API version: %s" % (e))
            raise e
