# -*- coding: utf-8 -*-
"""
Fortishield app - Report generation backend.
...
class PDF(FPDF):
    def header(self):
        ...
        # Logo
        self.image(
            '/opt/splunk/etc/apps/SplunkAppForFortishield/appserver/static/css/images/fortishield/png/logo.png',
            10, 10, 65, 0)
    ...
    def footer(self):
        ...
        self.copyright = 'Copyright © ' + str(self.year) + ' Fortishield, Inc.'
        self.set_y(-15)
        self.set_text_color(37, 107, 209)
...
class report(controllers.BaseController):
    ...
    def __init__(self):
        ...
        try:
            self.logger = log()
            self.path = '/opt/splunk/etc/apps/SplunkAppForFortishield/appserver/static/'
        ...
        try:
            ...
            for n in data['data']['configurations']:
                try:
                    ...
                    for currentSection in n['sections']:
                        ...
                        if 'groupConfig' in currentSection:
                            ...
                            if not conf_data or 'data' not in conf_data:
                                ...
                                self.setTableTitle(pdf)
                                # Get Fortishield version for the documentation link
                    ...
                    self.logger.error(e)
            # Save pdf
            pdf.output(self.path+'fortishield-'+pdf_name+'-'+report_id+'.pdf', 'F')
            self.logger.info('report agent configuration successful' +
                             self.path+'fortishield-'+pdf_name+'-'+report_id+'.pdf')
        except Exception as e:
    ...
    def generate(self, **kwargs):
        ...
        try:
            ...
            # Save pdf
            pdf_final_name = 'fortishield-'+pdf_name+'-'+report_id+'.pdf'
