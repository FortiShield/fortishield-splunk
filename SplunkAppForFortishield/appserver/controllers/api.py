...
class api(controllers.BaseController):
    ...
    def fortishield_ready(self, **kwargs):
        ...
        kwargs : dict
            Request parameters
        """
        self.logger.debug("api:is_fortishield_ready() called")
        try:
            api_id = utils.get_parameter(kwargs, 'apiId')
            api: API_model = API_services.get_api_by_id(api_id)

            daemons_ready = check_daemons(api)
            msg = "Fortishield is now ready." if daemons_ready else "Fortishield not ready yet."
            ...
            return jsonbak.dumps(
                {
                    "status": 200,
                    "ready": False,
                    "message": "Error getting the Fortishield daemons status."
                }
            )

    @expose_page(must_login=False, methods=['POST'])
    def request(self, **kwargs):
        """Make requests to the Fortishield API as a proxy backend.
    ...
    def autocomplete(self, **kwargs):
        """
        Provisional method for returning the full list of Fortishield API endpoints.
        """
        try:
