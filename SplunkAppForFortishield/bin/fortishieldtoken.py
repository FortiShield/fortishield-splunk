# -*- coding: utf-8 -*-
"""
Fortishield app - API backend module.
...
class fortishieldtoken():
    """
    FortishieldToken class.

    Handle the user's authorization token on the Fortishield API.
    """
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.error(
                "fortishield-token: error in the constructor: %s" % (e))

    def refresh(self):
        """
        Use this method to force a token refresh on the next request.
        """
        self.logger.debug("fortishield-token: token refresh requested")
        self.__refresh = True
    ...
    def get_auth_token(
        ...
        The token can be obtained from the session cache or he Fortishield Manager API.
        ...
        :return: String with the authorization token from the Fortishield API
        """
        self.logger.debug("fortishield-token::get_auth_token() called")
        if api is None:
            error = "Missing API object"
            self.logger.error(f"fortishield-token: {error}")
            raise Exception(error)
        else:
            self.logger.debug(
                "fortishield-token: API object provided" 
        ...
        try:
            ...
            if not self.__refresh and token is not None:
                self.logger.debug(
                    f"fortishield-token: the token for {token_key} key is in cache")
            # Otherwise, get a new token from the manager's API.
                ...
                # timeout of 900 seconds, as specified on the Fortishield API
                # TODO get configured value from /security/config endpoint
                self.cache.set(token_key, token, 900)
                self.logger.debug(f"fortishield-token: new token for key {token_key}")

            self.__refresh = False
            return token
        except Exception as e:
            self.logger.error("fortishield-token: error on get_auth_token: %s" % (e))
            raise e
    ...
    def _get_token_request(self) -> requests.models.Response:
        ...
        the Fortishield API to get the token. If allowed, an authorization context
        login is used.

        :return: Response object for the authorization request.
        """
        self.logger.debug("fortishield-token::get_token_request() called")
        try:
            ...
            if self.api.can_use_run_as():
                ...
                if response.status_code != 200:
                    self.logger.error(
                        f"fortishield-token: allow_run_as not enabled for user {self.api.get_auth()}\t"
                        + "API response:\t"
            ...
            self.logger.error("fortishield-token::get_token_request() - " + str(e))
            raise e
    ...
    def _basic_auth_login(self) -> requests.models.Response:
        ...
        """
        self.logger.debug("fortishield-token: using basic auth")
        try:
            ...
            self.logger.error(
                "fortishield-token: error on basic_auth_login: %s" % (e))
            raise e
    ...
    def _auth_context_login(self) -> requests.models.Response:
        ...
        self.logger.debug("fortishield-token: using auth context")
        try:
            return self.session.post(
            ...
            self.logger.error(
                "fortishield-token: error on auth_context_login: %s" % (e))
            raise e
