...
def check_daemons(api: API_model) -> bool:
    ...
    execd, modulesd, fortishielddb and clusterd
    ...
    if 'error' in daemons_status and daemons_status['error'] > 0:
        ...
        daemons = {
            "execd": d['fortishield-execd'],
            "modulesd": d['fortishield-modulesd'],
            "db": d['fortishield-db']
        }
        if cc:
            daemons['clusterd'] = d['fortishield-clusterd']
        values = list(daemons.values())
        # Checks all the status are equals, and running
        fortishield_ready = len(set(daemons.values())) == 1 and values[0] == "running"        

    # Log result
    if fortishield_ready:
        checked_debug_msg = "Fortishield daemons ready"
    else:
        checked_debug_msg = "Fortishield daemons not ready yet"
    logger.debug(
        "check_daemons(): %s" % checked_debug_msg
    )
    return fortishield_ready
